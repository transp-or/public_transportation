# Reusing results from an earlier case-study workflow

This document explains what can safely be reused when a case prepared with the
legacy public-package revision `478c473` is moved to the current
direct-scheduled workflow.

It is a migration and provenance document, not the operational walkthrough for
a new current-version case. Follow
[`new_case_study_walkthrough.md`](new_case_study_walkthrough.md) for the
current workflow.

## 1. Compatibility decision

The current package intentionally retired the former generic case-study runner,
`GenericCaseAdapter`, `GenericCaseHook`, and the reduced-OD response backend.
The current `main` revision is:

```text
7498d5e50f2dbd5ecad4d8d9721154141a0607b6
```

A case prepared with `478c473` must therefore choose one of two policies:

| Policy | Action |
|---|---|
| Finish the old run | Keep the private environment pinned to `478c473` and use its original driver and artifacts together. |
| Adopt current main | Preserve old artifacts read-only, create new current-version output roots, and rerun current preparation before preflight or fitting. |

Do not install current `main` into an environment that still expects the old
`run.py` and `adapter.py` contracts.

## 2. What can be reused

The following items can normally be carried forward, subject to a fresh audit:

| Item | Reuse policy |
|---|---|
| Raw timetable/scenario files | Reusable if checksums, service day, timezone, and source provenance are unchanged. |
| Raw measurement files | Reusable if event identifiers, timestamp semantics, and observation definitions are unchanged. |
| OD candidate source tables | Reusable as source data, but they must be loaded into the current canonical layout and revalidated. |
| Prior-demand values | Reusable as a scientific starting point if units, time-bin meanings, and OD ordering are unchanged. |
| Approved time-bin decisions | Reusable as scientific decisions, but current code must rematerialize and fingerprint the bins. |
| Structural-zero rules | Reusable as assumptions; current structural-zero preprocessing must rerun and produce new audit/fingerprint files. |
| Fixed-demand values | Potentially reusable after exact canonical-key reconciliation; never reuse merely by row order. |
| Measurement mapping decisions | Reusable as documented decisions, but current identifiers and mapping fingerprints must be regenerated. |
| Scientific model choices | Reusable as assumptions, with a new current-version model manifest. |
| Slurm resource observations | Reusable as planning evidence only; rerun current preflight because operator state and costs changed. |
| Old summaries and reports | Reusable for historical comparison and audit, not as current input artifacts. |

## 3. What cannot be reused as current preparation artifacts

The following legacy artifacts must not be passed to current APIs:

- `prepare_summary.json` from the generic runner;
- `results/artifacts/reduced_od/`;
- reduced-OD manifests, response atoms, route-choice caches, or
  reduced-OD fingerprints;
- checkpoints whose identity names the removed backend or old schema;
- old fit checkpoints when the current model/operator/layout fingerprint changes.

The current package has no automatic converter for those artifacts. Preserve
them in a read-only historical directory, for example
`results/legacy_478c473/`.

## 4. Safe side-by-side migration

1. Copy or archive the complete old case directory without modifying it.
2. Pin the new private environment to the current public commit.
3. Create new roots such as `results/current/`, `results/current/checkpoints/`,
   and `results/current/artifacts/`.
4. Rebuild the current scenario, canonical timetable, structural-zero output,
   fixed-demand reconciliation, compact OD layout, and measurement mapping.
5. Construct the current direct-scheduled operator and validate its complete
   identity-addressed manifest.
6. Run current preflight and the warm benchmark.
7. Start a new current fit with a new checkpoint identity.
8. Compare current results with the old summaries only after both workflows have
   completed; do not mix their artifacts during computation.

A successful migration does not require deleting or overwriting any old file.

## 5. Equivalence checks before recycling scientific inputs

For every candidate input that is being recycled, compare:

- file checksum and source provenance;
- canonical stop, trip, event, and OD/time identifiers;
- time-bin interval boundaries and interval semantics;
- measurement row count, ordering, type, stop, trip, and timestamp;
- fixed-demand keys and values;
- prior-demand units and total production by origin/time;
- structural-zero reason counts;
- current assignment, compact-layout, mapping, and model fingerprints.

If any comparison fails, treat the item as a new input and document the
difference. Never force compatibility by editing fingerprints.

## 6. Current completion criteria

A migrated case is ready for estimation only when the current workflow has:

- a complete structural-zero/fixed-demand output set;
- a complete direct-scheduled artifact manifest with matching identity;
- a completed current preflight;
- a finite warm value/gradient benchmark with acceptable agreement;
- a new current fit checkpoint/result and current model fingerprint.

The historical artifacts may remain beside these outputs, but they are not
part of the current computational dependency graph.

## 7. Preserved historical runbook

The remainder of this document retains the detailed `478c473` runbook for
reproducing old results. Its commands and imports apply only while the private
environment remains pinned to that revision.

## Historical 478c473 runbook (preserved for reference)
