...
expand=$(sbatch --parsable --dependency="afterok:${bins}" scripts/15_expand_od.sbatch)
...
printf '%s\n' \
  "probe=${probe}" "check=${check}" "universe=${universe}" "bins=${bins}" "expand=${expand}" "zeros=${zeros}" \
