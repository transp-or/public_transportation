...
| 2 | `test -f adapter.py && test -f run.py && test -f config.toml` | local | no | Complete case setup; missing files are a documentation/setup gap. |
...
| 4 | `uv run --frozen python -m public_transportation.preprocessing.time_discretization ...` | local | recommendation JSON only | Candidate bins, horizon, and budget are explicit; stop if any input is unknown. |
| 5 | `uv run --frozen python -m public_transportation.preprocessing.materialize_time_bins ...` | local | reviewed `time_bins.csv` | Reviewer adopts one candidate; stop if demand cannot be re-binned. |
