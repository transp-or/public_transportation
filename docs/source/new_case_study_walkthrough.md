...
The generic loader uses fixed *filename stems* inside the configured scenario
directory. The tabular extension may be `.csv`, `.parquet`, or `.json`; the
loader checks those extensions in that order. Thus, the package expects the
file to be named exactly `stops.csv` when the CSV representation is used, but
`stops.parquet` and `stops.json` are also supported alternatives. The
directory itself is configurable and is not hardcoded.

| File or filename pattern | Input/output | Required, optional, or generated | Package-hardcoded or user-configurable | Configuration key, if applicable | Stage that creates or consumes it |
|---|---|---|---|---|---|
| `metadata.json` | input | required | filename hardcoded; directory configurable | `[paths].scenario_directory` | `check`, all stages that load the scenario |
| `stops.(csv\|parquet\|json)` | input | required | stem hardcoded; extension alternative; directory configurable | `[paths].scenario_directory` | scenario loading, `check`, `od-universe`, `prepare` |
| `lines.(csv\|parquet\|json)` | input | required | stem hardcoded; extension alternative; directory configurable | `[paths].scenario_directory` | scenario loading, `check`, `prepare` |
| `time_bins.(csv\|parquet\|json)` | input | required source scenario periods | stem hardcoded; extension alternative; directory configurable | `[paths].scenario_directory` | scenario loading and base-data audit; approved bins are a later generated artifact |
| `trips.(csv\|parquet\|json)` and `stop_times.(csv\|parquet\|json)` | input | optional as a pair; if one exists, the other is required | stems hardcoded; extension alternative; directory configurable | `[paths].scenario_directory` | timetable loading, `check`, `od-universe`, `expand-od`, `prepare` |
| `demand.(csv\|parquet\|json)` | input | legacy-only; not required for independent OD | stem hardcoded when used as the scenario default; directory configurable | `[paths].scenario_directory` | legacy demand workflow only |
| `demand_flat.csv`, `demand_informed.csv` | input | optional, case-specific legacy/external files | not generic-loader requirements; names are examples unless a case adapter uses them | case-specific adapter/configuration | only the case-specific workflow that declares them |
| `<configured measurements file>` (for example, `inputs/measurements_boarding_alighting.csv`) | input | required | path configurable; the example filename is not universal | `[paths].measurements` | `check`, `prepare`, estimation diagnostics |
| `inputs/od_pairs.csv` (for example) | input | required when `[od_universe].source = "file"`; otherwise optional | path configurable; filename is an example | `[paths].od_pairs` or `[od_universe].pair_file` | `od-universe`, then `time-discretization` and `expand-od` |
| `inputs/prior_demand.csv` (for example) | input | optional; required when `[prior_demand].source = "external_file"` | path configurable; filename is an example | `[paths].prior_demand` / `[prior_demand]` | `expand-od` |
| `README.md` | case documentation | required by this walkthrough | filename required by the case-study convention; contents user-owned | none | case owner; reviewed before `check` |
| `adapter.py` | case adapter | required by the template workflow; normally copied unchanged | filename required by the template; contents user-owned only for documented hooks | none | imported by `run.py` |
| `run.py` | case dispatcher | required by the template workflow | filename required by the template; normally copied unchanged | none | invokes each stage |
| `pyproject.toml` | environment input | required | filename required by the case workflow; contents user-owned | none | `uv lock`, `uv sync`, all commands |
| `uv.lock` | environment input | required for reproducible frozen runs; generated by the case owner | filename required by `uv`; contents user-owned and must pin the public package | none | `uv sync --frozen`, provenance |
| `config/case.toml` | generic case configuration | required | filename required by the default `run.py`/runner; contents user-owned | `--config` may override the default | all stages |
| `config/time_discretization.toml` | time-bin contract | required by the generic template | filename and referenced path are configurable only through the case contract; contents user-owned | `[time_discretization].configuration_file` | `time-discretization`, `materialize-bins` |
| `config/reduced_od.toml` | reduced-OD preparation contract | required | filename is the template default; contents user-owned | `[model].configuration_file` | `prepare`, `preflight`, `fit` |
| `config/structural_zeros.toml` | legacy structural-zero contract | required by the template, although independent `expand-od` supplies its own timetable exclusions | filename is the template default; contents user-owned | `[structural_zeros].configuration_file` | legacy structural-zero tool or case-specific hooks |
| `config/model.toml` | model and optimizer settings | required | filename is the template default; contents user-owned | `[model].settings_file` | `check`, `preflight`, `fit` |
| `results/audit/input_audit.json` and `results/audit/run_manifest.json` | audit/provenance outputs | generated | filenames hardcoded by the generic runner | results directory comes from `[paths].results_directory` | `check` creates; later stages review fingerprints |
| `results/audit/od_pairs.csv`, `results/audit/od_universe_exclusions.csv`, `results/audit/od_universe.json` | canonical OD-universe outputs | generated | filenames hardcoded; output directory configurable | `[paths].results_directory` | `od-universe` creates; `time-discretization`/`expand-od` consume |
| `results/audit/time_discretization_recommendation.json` | audit output | generated | filename hardcoded; output directory configurable | `[paths].results_directory` | `time-discretization` creates; `materialize-bins` consumes |
| `results/generated_inputs/time_bins.csv` and `results/generated_inputs/time_bins_manifest.json` | approved generated input and manifest | generated | filenames hardcoded; output directory configurable | `[paths].results_directory` | `materialize-bins` creates; `expand-od` consumes only with matching fingerprints |
| `results/checkpoints/od_time_expansion/<fingerprint>/manifest.json`, `results/checkpoints/od_time_expansion/<fingerprint>/progress.json`, `results/checkpoints/od_time_expansion/<fingerprint>/chunk-*.jsonl` | expansion checkpoints | generated checkpoint files, not final canonical outputs | names hardcoded; root comes from `[paths].results_directory` | `[expansion]` controls chunking/checkpoint behavior | `expand-od` creates; `--resume` reuses a matching interrupted checkpoint |
| `results/generated_inputs/candidate_od_time.csv`, `results/generated_inputs/prior_demand.csv`, `results/audit/prior_generation.json` | canonical retained cells and prior outputs | generated | filenames hardcoded; output directory configurable | `[paths].results_directory`, `[prior_demand]` | `expand-od` creates; `structural-zeros`/`prepare` consume |
| `results/audit/production_attractiveness_provenance.json` | component-semantics audit | generated | filename hardcoded; output directory configurable | `[paths].results_directory`, `[model]` | `expand-od` creates; `prepare`/model review consumes |
| `results/audit/od_time_exclusions.csv` and `results/audit/od_time_expansion.json` | expansion audit outputs | generated | filenames hardcoded; output directory configurable | `[paths].results_directory` | `expand-od` creates; downstream stages validate |
| `results/generated_inputs/fixed_demand.csv`, `results/audit/structural_zero_audit.csv`, `results/audit/structural_zero_summary.json` | structural-zero outputs in the independent workflow | generated | filenames hardcoded; output directory configurable | `[paths].results_directory` | `structural-zeros` creates; `prepare` consumes |
| `results/structural_zeros/fixed_demand.csv`, `structural_zero_audit.csv`, `structural_zero_summary.json`, `resolved_config.toml`, `fingerprints.json` | outputs of the separate TOML structural-zero tool | generated only for that tool, not for the independent runner branch | filenames hardcoded by the tool; folder configurable | `[output].folder` in `config/structural_zeros.toml` | `run_structural_zero_preprocessing` creates |

The last row is deliberately separate: the candidate paths
`results/audit/resolved_config.toml` and `results/audit/fingerprints.json` are
not produced by the generic independent `structural-zeros` stage. The actual
filenames are written below the separate tool's configured output folder. Do
not document them as universal outputs of the independent stage.

In particular, the independent OD workflow does not require `demand.csv`,
`demand_flat.csv`, or `demand_informed.csv`. A legacy demand file is loaded
only when the case explicitly selects the legacy demand source. An externally
supplied OD-pair or prior-demand file is likewise optional and is admitted only
through its configuration key.

For example, a case may use:

```toml
[paths]
scenario_directory = "data/data_medium"
measurements = "data/data_medium/measurements_boarding_alighting.csv"
```

These paths are case-specific examples. They are not hardcoded package
requirements; the generic adapter resolves them from `config/case.toml`.

Before the first run, verify the configured file contract from the case-study
root. This checks the configured scenario directory, the fixed scenario stems,
the configured measurement file, all TOML contracts, and the reproducibility
files. It also warns if an input has accidentally been placed below the
generated-results directory:

```bash
CASE_ROOT="case_studies/<case_name>"
cd "$CASE_ROOT"
test -f adapter.py && test -f run.py && test -f pyproject.toml && test -f uv.lock
test -f config/case.toml && test -f config/time_discretization.toml
test -f config/reduced_od.toml && test -f config/structural_zeros.toml
...
config = load_case_study_config(root / "config/case.toml", case_root=root)
scenario = config.paths.scenario_directory
if not scenario.is_dir():
    raise SystemExit(f"missing configured scenario directory: {scenario}")
if not (scenario / "metadata.json").is_file():
    raise SystemExit(f"missing required scenario file: {scenario / 'metadata.json'}")
for stem in ("stops", "lines", "time_bins"):
    if not any((scenario / f"{stem}.{ext}").is_file() for ext in ("csv", "parquet", "json")):
        raise SystemExit(f"missing required scenario file stem: {stem}.(csv|parquet|json)")
trips = any((scenario / f"trips.{ext}").is_file() for ext in ("csv", "parquet", "json"))
stop_times = any((scenario / f"stop_times.{ext}").is_file() for ext in ("csv", "parquet", "json"))
if trips != stop_times:
    raise SystemExit("trips and stop_times must be supplied together")
if not config.paths.measurements.is_file():
    raise SystemExit(f"missing configured measurement file: {config.paths.measurements}")
for path in config.package_config_paths:
    if not path.is_file():
        raise SystemExit(f"missing configuration contract: {path}")
results = config.paths.results_directory.resolve()
for label, path in (("measurements", config.paths.measurements), ("scenario", scenario)):
    if results == path.resolve() or results in path.resolve().parents:
        raise SystemExit(f"{label} appears under generated results: {path}")
print("configured inputs and case-owned contracts are present")
PY
```

Do not treat generated files such as `results/generated_inputs/time_bins.csv`
or `results/generated_inputs/candidate_od_time.csv` as raw scenario inputs.

The checkpoint files are not final outputs. Completed chunks survive an
interruption and may be reused only with an explicit `--resume`; the canonical
CSV files are published only after all chunks complete. A changed
configuration, package revision, source checksum, OD universe, approved time
bins, or feasibility setting changes the fingerprint and invalidates reuse.
...
|---|---|---|
| `results/audit/od_pairs.csv`, `results/audit/od_universe_exclusions.csv`, `results/audit/od_universe.json` | `od-universe` | `expand-od`, review/audit |
| `results/generated_inputs/time_bins.csv`, `results/generated_inputs/time_bins_manifest.json` | `materialize-bins` | `expand-od` |
| `results/checkpoints/od_time_expansion/<fingerprint>/manifest.json`, `results/checkpoints/od_time_expansion/<fingerprint>/progress.json`, `results/checkpoints/od_time_expansion/<fingerprint>/chunk-*.jsonl` | `expand-od` | `expand-od --resume`; not consumed as final canonical inputs |
| `results/generated_inputs/candidate_od_time.csv`, `results/audit/od_time_exclusions.csv`, `results/audit/od_time_expansion.json` | `expand-od` | `structural-zeros`, `prepare` |
| `results/generated_inputs/prior_demand.csv`, `results/audit/prior_generation.json` | `expand-od` | `prepare`, model provenance |
| `results/generated_inputs/fixed_demand.csv`, `results/audit/structural_zero_audit.csv`, `results/audit/structural_zero_summary.json` | `structural-zeros` | `prepare` |
...
def load_case(case_root: Path):
    ...
    config = load_case_study_config(case_root / "config/case.toml", case_root=case_root)
    scenario = Scenario.from_folder(config.paths.scenario_directory, strict=True)
...
Here `measurements_boarding_alighting.csv` is only a conventional example
filename. The generic adapter reads the path configured by
`[paths].measurements`; it does not expect a measurement file under
`results/audit/` and does not require that particular basename.
...
`expand-od` writes `results/generated_inputs/candidate_od_time.csv`,
`results/audit/od_time_expansion.json`, and
`results/audit/od_time_exclusions.csv`. A physically
...
only when its companion `results/generated_inputs/time_bins_manifest.json`
...
MEASUREMENTS_PATH="data/data_medium/measurements_boarding_alighting.csv"  # for example
shasum -a 256 "$MEASUREMENTS_PATH"
```

The `MEASUREMENTS_PATH` assignment is case-specific; in a real case set it to
the same path configured by `[paths].measurements`. The basename
`measurements_boarding_alighting.csv` is only a conventional example, not a
hardcoded package requirement.
...
- `results/generated_inputs/fixed_demand.csv`;
- `results/audit/structural_zero_audit.csv`;
- `results/audit/structural_zero_summary.json`;
- for the separate TOML structural-zero tool, the configurable output folder
  (the template default is `results/structural_zeros/`) contains
  `resolved_config.toml` and `fingerprints.json`;
...
`config/structural_zeros.toml`, and `config/model.toml`. The separate
structural-zero tool writes `resolved_config.toml` and `fingerprints.json` under
the configured `[output].folder` (the template default is
`results/structural_zeros/`); they are not root-level configuration contracts
and are not produced at `results/audit/` by the independent runner branch. No
command may assume a root-level legacy configuration file.
