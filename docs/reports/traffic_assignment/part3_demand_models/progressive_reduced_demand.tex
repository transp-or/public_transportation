...
\cite{CameronTrivedi2013}.  ZIP and ZINB add a separate Bernoulli zero state
using stable log-space mixtures \cite{Lambert1992}.  This state is not a
timetable structural zero: network-infeasible cells have already been removed,
...
ZIP/ZINB & design columns & Separate zero process & Low & Mean/zero confounding & Excess zeros after NB\\
\hline
\end{tabular}
...
modes; vector or canonical named bounds act in raw parameter coordinates.
An absolute time budget requires an atomic checkpoint.  A stopped run records
the latest accepted iterate and resumes only when the complete model identity
...
Each accepted iteration additionally reports its duration, a rolling mean over
at most ten iterations, the remaining iteration allowance, estimated remaining
seconds, and the expected UTC completion time.  The estimate excludes JAX
compilation and is therefore meaningful only after several iterations.  With a
configured checkpoint, progress includes its path and age; an interactive
interrupt atomically saves the latest accepted iterate and returns an explicit
``interrupted'' status suitable for later identity-validated resume.
