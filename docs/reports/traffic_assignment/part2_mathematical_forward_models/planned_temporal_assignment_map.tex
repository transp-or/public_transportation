\section{Planned Reusable Temporal Assignment-Map Architecture}
\label{sec:planned-temporal-assignment-map}

This section is a design specification and implementation roadmap.  It extracts
a stable computational architecture from the forward models developed earlier
in this part, especially the scheduled time-expanded formulation, the reduced
journey-response formulation, the additive fixed-routing decomposition, and the
measurement model.  It deliberately distinguishes verified public-library
functionality from proposed work.  In particular, it does not specify an
alternative supply model: the first target backend remains scheduled,
time-expanded assignment.
...
\item[\textbf{IMPLEMENTED}] The public library contains a concrete, tested
  implementation of the stated behavior.
...
\textbf{PARTIALLY IMPLEMENTED.}  The public library already composes gravity
demand generation, measurement operators, likelihoods, and optimizers in this
form.  Several operator representations are interchangeable within existing
workflows.  A single public artifact implementing the complete temporal design
specified below does not yet exist.
...
\paragraph{Implementation status.}
\textbf{PARTIALLY IMPLEMENTED.}  Tested linear-operator and gravity-measurement
protocols expose forward and adjoint products, shapes, fingerprints, and
operational capabilities.  A canonical artifact identity now separates
physical indexing, supply, timetable, temporal, choice, feasibility,
measurement, coefficient-policy, numerical, and schema dependencies, with
explicit mismatch diagnostics.  Existing fixed-routing operators validate
against this identity through an adapter.  The complete dependency boundary
is not yet embodied by one persisted scheduled assignment artifact.
...
\paragraph{Implementation status.}
\textbf{PARTIALLY IMPLEMENTED.}  Existing reduced-OD and fixed-routing paths
use immutable layouts, explicit dimensions, fingerprints, schemas, and
fail-closed cache validation.  Immutable canonical interval, physical OD-time,
and boarding/alighting measurement tables are now implemented, including
separate physical-artifact and free/fixed-binding fingerprints.  Their builder
adapts the existing OD parameter layout and validates fixed-routing bindings.
These tables are not yet populated and consumed by every assignment backend
and every demand-model workflow.
...
\paragraph{Implementation status.}
\textbf{UNIMPLEMENTED.}  The library accepts temporal layouts in existing
pipelines, but it does not generally select a data-driven non-uniform partition
or construct the Pareto frontier described by
...
\textbf{PARTIALLY IMPLEMENTED.}  The library can derive an exact, independently
...
\paragraph{Implementation status.}
\textbf{PARTIALLY IMPLEMENTED.}  Exact in-memory temporal blocks now merge duplicates,
...
\paragraph{Implementation status.}
\textbf{PARTIALLY IMPLEMENTED.}  The generic reduced-demand path implements
Poisson, negative-binomial, zero-inflated Poisson, and zero-inflated
negative-binomial contributions independently of routing, and automatic
differentiation composes them with operator products.  A stable general
observation-model protocol shared by every estimation path is not yet defined.
...
`-- benchmarks/
\end{verbatim}
The manifest should record a schema version, canonical-index fingerprints,
network and timetable fingerprints, temporal discretization, choice-parameter
fingerprints, feasibility constraints, coefficient precision and threshold,
dimensions, total nonzeros, block dimensions, and construction diagnostics.
It must not contain a gravity specification, prior, likelihood, or optimizer
configuration.  Payloads should be published atomically and validated before
reuse.  Incompatible or incomplete artifacts must fail closed.

\paragraph{Implementation status.}
\textbf{PARTIALLY IMPLEMENTED.}  The library now has a content-addressed,
...
selectively before atomic final publication.  The fuller directory above,
including separate diagnostic and benchmark products inside the final artifact,
...
\paragraph{Implementation status.}
\textbf{PARTIALLY IMPLEMENTED.}  The foundations identified in the table are
available, but the remaining priorities are still a roadmap.  The temporal
CSR/CSC backend is now implemented and tested for forward, adjoint, JIT, and
automatic-differentiation use.  The scheduled constructor remains experimental
until contributions are emitted directly by alternative preprocessing rather
than by batched unit-response evaluation.
...
a presentation-layer convention. Physical-stop, route-pattern, service-period
and timetable indexing, bounded RAPTOR rounds and destination scans,
journey-choice construction, measurement-response aggregation,
origin-support discovery, structural-zero classification, and artifact
publication report completed and total units, elapsed time, observed
...
\paragraph{Implementation status.}
\textbf{PARTIALLY IMPLEMENTED.}  Current public workflows already embody
forward/adjoint products, separation from several demand and likelihood
choices, progress reporting, and resumable long-running preprocessing.  The
invariants become collectively enforceable only after the canonical contract,
index tables, and complete scheduled temporal artifact are implemented and
tested together.
