...
uses the optimizer-only Biogeme TR--BFGS trust-region method by default.  The
explicit SciPy L--BFGS--B comparison or compatibility path remains available.
The execution setting
\texttt{optimizer\_maxls} controls the maximum number of line-search steps
(20 by default).  The optimizer-only
\texttt{biogeme-optimization} distribution provides the comparison API; the
full Biogeme modeling package is not required.  Optimizer and progress
...
The default Biogeme TR--BFGS gravity fit is not a bounded optimization in raw
coordinates.  The SciPy L--BFGS--B comparison path also does not enforce
bounds; its settings must be treated as part of that comparison's declared
configuration, not assumed to apply to the default estimator.
