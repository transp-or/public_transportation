...
For exploratory optimization steps, the gravity layer can request an effort
level between 1 and 100.  Routing-cache shards are selected by immutable,
seeded hashes within declared strata, so selections are deterministic, nested,
and reproducible.  Selection probabilities include mandatory nonempty-stratum
representation, and inverse probabilities provide Horvitz--Thompson expansion
weights.  Realized support entries and routing bytes are reported because
unequal shard sizes can make effective effort differ from the request.
...
Deterministic replicate groups estimate objective, predicted-count, gradient-
norm, and gradient-direction errors without an exact reference or repeated
routing products.  Measurement coverage and realized support also enter a
quality score in $[0,1]$.  These quantities are diagnostic estimates, not
guaranteed bounds, and shared sampling bias can make replicate uncertainty
optimistic at low effort.  A reusable anchor can instead estimate routed-demand
differences around an exact or documented-fidelity prediction.  Operator,
routing-cache, parameter, package, and schema identities prevent incompatible
reuse.  Atomic checkpoints preserve the subset, weights, accumulators, quality
groups, and anchor identity; cancellation or deadline expiration never
publishes a partial result as complete.

A public dense benchmark with 200 OD cells, 60 measurements, and 32 synthetic
shards shows improving accuracy but no runtime gain: fixed dispatch and quality
overhead dominate the inexpensive dense multiplication.  At approximately
76\% realized support, gradient relative error is about 3.1\% and gradient
cosine similarity is 0.99994, whereas very-low-effort gradients can point in
the wrong direction.  Large persisted routing caches require a separate timing
campaign before any speedup is claimed.  A conservative future trust-region
policy should escalate fidelity when coverage is incomplete, quality is below
0.70, estimated relative gradient error exceeds 0.10, or cosine confidence is
below 0.90, and must require effort 100 before convergence is declared.
