...
The gravity objective now consumes a device-native operator protocol rather
than an explicit matrix.  Dense and BCOO operators retain their stored matrix
for compatibility, while a matrix-free implementation exposes JAX forward,
transpose, and bounded multiple-right-hand-side products backed directly by
fixed-routing loading and measurement aggregation.  Thus a large logical
...
residency.  Deterministic forward and reverse topological passes accumulate
measurements and OD cotangents one shard at a time; no complete routing array,
link-flow-by-shard stack, Jacobian, or measurement-by-OD matrix is assembled.
Structured progress records shard and group completion, cache reuse, elapsed
and estimated remaining time, memory, cache size, and deadline allowance.  On
