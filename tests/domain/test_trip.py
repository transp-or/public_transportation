...
def test_negative_capacity_is_error_with_context():
    tr = Trip(trip_id="TR1", capacity=-1)
    rep = tr.validate()
...
def test_capacity_none_has_no_issues():
    tr = Trip(trip_id="TR1", capacity=None)
    rep = tr.validate()
    assert rep.issues == []
...
def test_negative_direction_id_is_error_with_context():
    tr = Trip(trip_id="TR1", direction_id=-1)
    rep = tr.validate()
    ...
    iss = issues[0]
    assert iss.severity == Severity.ERROR
    assert iss.location == "timetable.trips[TR1].direction_id"
    assert iss.context is not None
    assert iss.context["direction_id"] == -1


def test_direction_id_zero_or_positive_ok():
    tr0 = Trip(trip_id="TR1", direction_id=0)
    tr1 = Trip(trip_id="TR2", direction_id=1)
    rep0 = tr0.validate()
    rep1 = tr1.validate()
    assert rep0.issues == []
    assert rep1.issues == []
...
def test_multiple_issues_are_all_reported():
    tr = Trip(trip_id="", capacity=-5, direction_id=-2)
    rep = tr.validate()
    codes = _codes(rep)
