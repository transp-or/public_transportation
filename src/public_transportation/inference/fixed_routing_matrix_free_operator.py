...
class MatrixFreeFixedRoutingMeasurementOperator:
    ...
    def __post_init__(self) -> None:
        ...
        def forward(free_demand: jnp.ndarray) -> jnp.ndarray:
            ...
            active = active.at[free_indices].set(free_demand)
    ...
    def _compile_forward(self, value: jnp.ndarray) -> None:
        ...
        started = self.clock()
        self._forward_product = jax.jit(self._forward_function).lower(value).compile()
    ...
    def _compile_transpose(self, value: jnp.ndarray) -> None:
        ...
        started = self.clock()
        self._transpose_product = (
            jax.jit(self._transpose_function).lower(value).compile()
        )
